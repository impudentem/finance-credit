...
module.exports = {
  ...
  addDerivedValues: function(config) {
    ...
    // takes component object and creates file glob matching selected components
    config.globs.user_components = (typeof config.user_components == 'object')
      ? (config.user_components.length > 1)
        ? '{' + config.user_components.join(',') + '}'
        : config.user_components[0]
      : '{' + defaults.user_components.join(',') + '}'
    ;
